...
class PlantRow extends React.PureComponent {
  ...
  formatIntervalAsString(interval) {
    ...
    const secondsInTheDay = 24 * 60 * 60; // 24hours * 60 minutes * 60 secods
    const miliSecondsInSecond = 1000;
    const days = Math.round(interval / secondsInTheDay);
    const formatted = moment.utc(interval * miliSecondsInSecond).format(`${days}:HH:mm:ss`);

    return `Formatted seconds: ${formatted}, All seconds: ${interval}`;
  }
